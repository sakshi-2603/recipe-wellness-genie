...
/**
 * Filters fallback recipes based on user preferences
 */
function filterFallbackRecipes(preferences: UserPreferences): Recipe[] {
...
/**
 * Verifies if the OpenAI API key is valid
 */
async function verifyOpenAIKey(): Promise<{
  isValid: boolean;
  message: string;
}> {
...
/**
 * Builds the prompt for OpenAI based on user preferences
 */
function buildOpenAIPrompt(preferences: UserPreferences): string {
  ...
  // Create the prompt
  return `Generate 3 healthy recipes based on the following preferences:
...
/**
 * Generates recipes using OpenAI API
 */
async function generateOpenAIRecipes(preferences: UserPreferences): Promise<{
  recipes?: any[];
  error?: string;
}> {
  ...
  try {
    const prompt = buildOpenAIPrompt(preferences);
    ...
    return { recipes: parsedRecipes.recipes };
  } catch (error) {
...
/**
 * Handle API test mode for verifying API key
 */
async function handleTestMode(): Promise<Response> {
  const keyStatus = await verifyOpenAIKey();
...
/**
 * Handle normal recipe generation mode
 */
async function handleRecipeGeneration(preferences: UserPreferences): Promise<Response> {
  try {
    // Try OpenAI API first
    const { recipes, error } = await generateOpenAIRecipes(preferences);
...
serve(async (req) => {
  ...
  try {
    ...
    if (testMode) {
      ...
      // Normal mode - generate recipes
      return await handleRecipeGeneration(preferences);
    }
  } catch (error) {
